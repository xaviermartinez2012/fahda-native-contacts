...
######################################################################################################################
$input2 = "/home/server/FAHdata/PKNOT/1-NativeContacts//1796_1798_all-luteo-data.log";
$output = "/home/server/FAHdata/PKNOT/1-NativeContacts/2-FindNativeSims/luteo.cutoff3.75.native.sims.incduling-t0.txt";
$output2 = "/home/server/FAHdata/PKNOT/1-NativeContacts/2-FindNativeSims/luteo.cutoff4.25.native.sims.including-t0.txt";
$output3 = "/home/server/FAHdata/PKNOT/1-NativeContacts/2-FindNativeSims/luteo.cutoff5.native.sims.including-t0.txt";
...
for ($j=0; $j<$totrow;$j++){
	...
	do {    $proj = $data[$j][0];
		$run = $data[$j][1];                                  # for every simulation set... per clone
		$clone = $data[$j][2];
		$time = $data[$j][3];	                         # set values
		if ($time >= 0.0 && $run == 0 && $proj <= 1797 ) {   $rmsd = $data[$j][4];                 # checking for native state under 4
			if ($rmsd <= 3.750) { $count  = "yes"; 
				    push @totaltime, $time;
				    push @resp, $count;
			}
...
############# cutoff 4
	for ($c=0;$c<$totresp;$c++){
		if ($resp[$c] eq "no" ) { $shouldWePrint = "no"; last; }  #checks if there is a no
	}
	if ($shouldWePrint eq "yes") {  print OUT "$data[$j][0]	  $data[$j][1]   $data[$j][2]	$data[$j][3]\n"; }  # prints out run and clone if all frames in simulation were native
########## cutoff 4.5
...
########## cutoff 4.5
	for ($y=0;$y<$totresp3;$y++){
		if ($resp3[$y] eq "no" ) {$shouldWePrint3 = "no"; last; }  #checks if there is a no
		else {	
			$shouldWePrint3 = "yes";        # Initializes print to yes;
		}

	}
	if ($shouldWePrint3 eq "yes") { print OUT3 "$data[$j][0]   $data[$j][1]   $data[$j][2]   $data[$j][3]\n"; }  # prints out run and clone if all frames in simulation were native
