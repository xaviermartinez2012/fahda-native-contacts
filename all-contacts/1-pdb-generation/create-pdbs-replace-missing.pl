...
# ------------------------------------------------------------------------------
# Generate PDBs from missing pdb files list
...
$MISSING_PDB_LIST = "/home/server/FAHdata/PKNOT/1796.log";
$DELTA_RESIDUES   = 4;
$MAX_DISTANCE     = 6;

# Read ZERO files
open(my $pdbList, '<', $MISSING_PDB_LIST) || die "No PDB file";
while (my $line = <$pdbList>) {
    chomp($line);
    ...
    generate_frames_pdb($curDir, $fileName);

    # Determine how many PDB files were generated
    $filepath    = "${curDir}*.pdb";
...
sub generate_frames_pdb {
    my ($curDir, $fileName) = @_;
    my $xtcFileName = uc($fileName);
    my $xtcFile     = "${curDir}${xtcFileName}.xtc";
    my $tprFile     = $curDir . 'frame0.tpr';
    print STDOUT "$curDir \n $xtcFile\n $tprFile\n";
    ...
    # Generate all PDB files that have duplicates pdb files
    ...
    $frame = 0;
    for ($i = 0 ; $i <= $lastpdbfile ; $i++) {
        open(my $pdbIn, '<', $pdbFile . '_' . $i . '.pdb') || die "No pdb file";
        while ($tmpline = <$pdbIn>) {
            chomp($tmpline);
...
sub find_native_contacts {
    ...
    open(my $W, '>', $conFile) || die "Please give me output filename $!";
