...
$DATA_DIR      = '/home/server/FAHdata/PKNOT/';
$ZERO_CON_LIST = '/home/server/FAHdata/PKNOT/check_FAH_CON-files_1799.log';
$PROJ          = 1799;
...
#read ZERO files
open(my $pdbList, '<', $ZERO_CON_LIST) || die "No pdb file";
while ($line = <$pdbList>) {
    chomp($line);
    ...
    if (substr($lines[0], $cIndex + 2, $fIndex - $cIndex - 2) ne $clone) {
        ...
        $curDir   = $DATA_DIR . "PROJ" . $PROJ . '/RUN' . $run . '/CLONE' . $clone . '/';
...
sub cp_pdb_has_no_con {
    $mkcommand = $DATA_DIR . 'PDB_with_empty_Con_Files/';
    `mkdir $mkcommand`;
