...
# READING IN & STORING THE LIST CONTAINING NATIVE STATE SIMULATIONS
	...
	while(my $line = <NSFILE>)
	{
		...
		# the array below should contain: (0) project, (1) run, 
		# (2) clone, (3) total time in ps
		@lineInNatSims = split(/ /,$line);
	...
	if ($NSindex == 0)
	{
		print "FATAL ERROR: There are no native simulations being used, ";
		print "make sure native simulation file $natSimFile exists and is not empty.\n";
		exit;
	}
	else 
	{
		print "\nNative sim file $natSimFile contains usable data. ";
		print "Reading in contact file $allConFile now...\n";
	}
...
# COMPARE & PRINT NATIVE SIMS CONTACTS
	...
	# it is a timestamp if the first letter of the string is "p"
		...
		{ 
			...
			for ($i = 0; $i < $NSindex; $i++) 
			{
				if (($ProjRunClone[0] eq $nativeSims[$i]) and 
					($ProjRunClone[1] ne "0.con")) 
				{ 
					$shouldWePrint = 1;
