...
# ================================================================================================
#  Summing up categorized native contacts for entire project per time stamp    By: Arad & Khai 1/13/14
# ================================================================================================


#`echo "$0 starts at $timeStart" | mail -s "server at sorin2 message" nguyenkhai101\@gmail.com`;

# ================================================================================================
# INPUT
	$usage = "\nUsage: perl script\.pl [project] [nat-con-file] [all-contact-file] [percent] [distance] [distanceNC] [output-file] [exclude-list-output-file] [native-contacts-list-output-file]\n";
...
# INITIALIZE HASH TABLES
	$numAtom       = 839; # number of atoms there are for the molecule, must be changed for every new molecule
	for($i=1; $i < $numAtom; $i++){
		for ($j=1; $j < $numAtom ; $j++){
			$nativeSim{"$i:$j"} = 0;
			$meanDista{"$i:$j"} = 0;
...
# EXTRACTING INFORMATION ABOUT PROJ, RUN, CLONE, AND TIME
	while ($line2 = <inCON>){
		...
		@line2     = split(/ /,$line2);
		@line2Char = split(//,$line2); # split line into characters in order to detect c3eyhange in frame
...
# STARTING TO EVALUATE CONTACTS
	...
	if ($numColumn == 10) {   # to check if the input concatenated all-contact data file is valid
		...
		if ($dist <= $distance) {
			...
			elsif ($nativeSim{"$atom1:$atom2"} == 0 && $exlu{"$atom1:$atom2"} == 0){
				$nonNC++;
			}
		}
	}
	...
	if ($shouldWePrint==1){
		...
		$s1=0; $s2=0; $l1=0; $l2=0; $t=0; $totNC=0; $nonNC=0; # reseting counters for next time frame
		$shouldWePrint=0;
	}
	$count++;
} # end of `while` loop
...
#`echo "$0 starts at $timeEnd" | mail -s "server at sorin2 message" nguyenkhai101\@gmail.com`;
# ================================================================================================
